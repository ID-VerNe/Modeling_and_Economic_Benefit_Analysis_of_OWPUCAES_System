...
class 风电站(object):
    def __init__(self, 容量, 发电阈值, 上一轮发电量, 轮次=2, 是否输出=False):
        self.容量 = 容量
        self.发电阈值 = 发电阈值
        self.上一轮发电量 = 上一轮发电量
        self.轮次 = 轮次
    ...
    def 发电量(self, 随机等级=3, 输出方法=0):  # 输出方法=1：不含阈值  ；  输出方法=2：含有阈值
    ...
    def 输出并返回总参数(self):
        参数集合 = {
            '名字': self.名字,
            '轮次': self.轮次,
            '本轮发电量': self.发电量()['本轮发电量'],
            '带阈值的发电量数值': self.发电量()['带阈值的发电量数值'],
            '发电阈值':self.发电阈值
        }
        if self.是否输出 == True:
            基本公式.表格输出(参数集合)
...
class 电动机(object):
    ...
    def __init__(self, 转速, 输入电量, 效率, 序列号, 是否输出=False):
        self.转速 = 转速
        self.输入电量 = 输入电量
        self.效率 = 效率
        self.名字 = '电动机 #' + str(序列号)
...
class 空气压缩机(object):
    ...
    def __init__(self, 入口温度, 入口压强, 空气流速, 压缩效率, 序列号, 电动机输入功, 是否输出=False):
        self.入口温度 = 入口温度
        self.入口压强 = 入口压强
        self.空气流速 = 空气流速
        self.压比 = 2.1544
    ...
    def 真实流速(self):
        if self.序列号 == 1:
            标幺值 = self.电动机输入功 / 0.79
        elif self.序列号 == 2:
            标幺值 = self.电动机输入功 / 0.81
        elif self.序列号 == 3:
            标幺值 = self.电动机输入功 / 0.84
        # print(标幺值)
        return 标幺值
    ...
    def 出口温度(self):
        # 出口温度数值 = self.入口温度 * (1 + (1 / self.真实压缩效率()) * self.压比 ** ((self.热交换系数r - 1) / (self.热交换系数r)))
        出口温度数值 = self.入口温度 + self.入口温度 * (1 / self.真实压缩效率()) * (self.压比 ** ((self.热交换系数r - 1) / self.热交换系数r) - 1)
        return 出口温度数值
    ...
    def 压缩机气体能量变化量(self):
        能量变化量 = self.空气流速 * self.真实流速() * (基本公式.空气比焓(空气温度=self.入口温度) - 基本公式.空气比焓(空气温度=self.出口温度()))
        while abs(能量变化量) >= self.电动机输入功:
            能量变化量 = 能量变化量 * 0.9
        return 能量变化量

    def 真实空气流速(self):
        return self.真实流速() * self.空气流速
...
class 压缩机热交换器(object):
    ...
    def __init__(self, 入口温度, 空气流速, 冷却液温度, 冷却液流速, 序列号, 压缩机输入热量, 是否输出=False):
        self.入口温度 = 入口温度
        self.空气流速 = 空气流速
        self.冷却液温度 = 冷却液温度
        self.冷却液流速 = 冷却液流速
    ...
    def 出口温度(self):
        换热器出口温度 = self.入口温度 - self.热交换有效度epsilon * (self.入口温度 - self.冷却液温度)
        return 换热器出口温度
...
class 储气包(object):
    储气包深度 = 100
    温度 = 290
    压强 = 2.1544 ** 3 * 大气参数.空气压强

    def __init__(self, 上一轮容量, 入口空气流速, 出口空气流速, 是否输出=False):
        self.上一轮容量 = 上一轮容量
        self.入口空气流速 = 入口空气流速
        self.出口空气流速 = 出口空气流速
        self.名字 = '储气包'
...
class 空气膨胀机(object):
    ...
    def __init__(self, 入口温度, 入口压强, 空气流速, 膨胀比, 膨胀效率, 序列号, 是否输出=False):
        self.入口温度 = 入口温度
        self.入口压强 = 入口压强
        self.空气流速 = 空气流速
        self.膨胀比 = 膨胀比
...
class 膨胀机热交换器(object):
    ...
    def __init__(self, 入口温度, 空气流速, 冷却液温度, 冷却液流速, 序列号, 压缩机热交换器输入热量, 是否输出=False):
        self.入口温度 = 入口温度
        self.空气流速 = 空气流速
        self.冷却液温度 = 冷却液温度
        self.冷却液流速 = 冷却液流速
...
class 发电机(object):
    ...
    def __init__(self, 输入电量, 效率, 序列号, 是否输出=False):
        self.输入电量 = 输入电量
        self.效率 = 效率
        self.名字 = '发电机 #' + str(序列号)
        self.是否输出 = 是否输出
...
class 储热罐(object):
    ...
    def __init__(self, 上一轮温度, 导热油流速, 压缩机换热器输出的热量, 是否输出=False):
        self.名字 = '储热罐'
        self.上一轮温度 = 上一轮温度
        self.导热油流速 = 导热油流速
...
class 储冷罐(object):
    def __init__(self, 上一轮温度, 导热油流速, 膨胀机换热器输出的热量, 是否输出=False):
        self.名字 = '储冷罐'
        self.上一轮温度 = 上一轮温度
        self.导热油流速 = 导热油流速
